...
function mutate!(edges, strength = 10)
    for i = 1:length(edges)
        edges[i] += strength * (rand() * 0.003 - 0.0015)
        #if edges[i] < 0
        #    edges[i] = 0.1
        #end
    end
...
function create_mutations(edges)
    combinations = []
    costs = []
    for i in 1:10
        mut_edges = copy(edges)
        mutate!(mut_edges)
        mutate!(mut_edges)
        mutate!(mut_edges)
        @info mut_edges[1]
        this_sim = new_sim()
        set_edges!(this_sim, mut_edges)
        simulate!(this_sim, 0.01, 15000, false)
        @info "" this_sim[30].pos
        c = cost(this_sim)
        @info "" c
        push!(combinations, mut_edges)
        push!(costs, c)
    end
    return (combinations, costs)
end
...
function main()
    ...
    if _draw
        ...
        global pp = []
        global springs = Tuple.(edges(sim))
        for edge in springs
    ...
    global best_edges = copy(new_edges)
    @info "test"
    for i = 1:4
        (combinations, costs) = create_mutations(best_edges)
        println(costs)
        index = argmin(costs)
        best_edges = combinations[index]
        println(i, " cost: ", costs[index])
        # begin
        #     global combinations = []
        #     global costs = []
        #     for j in 1:10
