...
function draw(network::Network)
    ...
    for (n1, n2) in keys(network.graph.edge_data)
        xs = Observable([get_neuron(network, n1).pos[1], get_neuron(network, n2).pos[1]],
        # ignore_equal_values=true
)
        push!(edge_draw_obs, xs)
        ys = Observable([get_neuron(network, n1).pos[2], get_neuron(network, n2).pos[2]],
        # ignore_equal_values=true
)
    ...
    return scatter!(xs, ys; marker=:circle, markersize=25, color=:blue)
end
...
function Visualizer(network::Network; max_fps::Number=10)
    ...
    for (n1, n2) in edge_pairs
        ...
        l = lines!(
            vis.observers[n1, n2, :xs], vis.observers[n1, n2, :ys]; color=:red, stroke=10
        )
    end
